	...
	case 1:
		{
			for (int i = j;i == j; i++)
			{
				printf("Ingrese el n%cmero del art%cculo corregido: \n", 163, 161);
				scanf_s("%d", &tienda[i].id);
				while (getchar() != '\n');
			}
			break;
...
{
	int j,k;
	printf("¿Cu%cl de los registros desea eliminar de la existencia?", 160);
	scanf_s("%d", &j);
	k = j;
	j = j - 1;
	for (int i = j;i == j;i++)
	{
		printf(" El registro %d ese ha ido de sab%ctico, cari%co. . .",k, 160,164);
		tienda[i].id = 0;
	}
}
